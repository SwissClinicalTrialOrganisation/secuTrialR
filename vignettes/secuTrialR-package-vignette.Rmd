...
There is one princtiple function to read your data (i.e. `read_secuTrial()`).
...
cryptic to you we suggest you talk so someone who has implemented a data base in secuTrial and let them explain it with a
specific example.
...
Note that descriptive labels have also been added to the data.
```{r}
...
Datetime data is also transformed and similarly to the factorization process appended with
`.date` or `.datetime`.
...
Much more information is stored in the elements of `export_options` and their names should be descriptive regarding their contents.
...
Now that you understand the `secuTrialdata` object we will show you some generic analyses you can perform with objects of this class.
...
```{r}
plot_recruitment(ctu05_data, cex = 1, rm_regex = "\\(.*\\)$")
```
...
how well the forms have been filled. The below table shows both absolute and relative numbers for all forms.
...
completeness on variable basis while showing 0% completeness on form basis. Unfortunately it is currently not
possible to assess completeness on variable basis generically. Hence, high completeness on form basis implies
...
return_random_participants(ctu05_data, percent = 0.25, seed = 1337, date = "2019-03-18",
...
During ongoing studies it is possible that changes to the secuTrial data entry interface are made. Sometimes
these changes may call for adjustments in analysis code. It is considered good practice to run 
`diff_secuTrial()` on the last export and the current export of a project to at least make yourself
aware of potential changes to the setup. If there are differences, the results of this function should
...
Since this list contains only empty entries you can conclude that nothing has changed.

## Conversion to SPSS, STATA, SAS

Since you are working with R it is unlikely that you need such conversions for yourself. However,
...
Since this is not heavily tested there may be issues and you might prefer doing this manually with the `haven` package.
...
like to prepare a subset of the data for a nested study. We have implemented this option with `subset_secuTrial()`.
It will truncate your `secuTrialdata` object and return a new `secuTrialdata` object. It is possible to subset by
including or excluding specific participant ids or centres.
...
If you subset based in centres all traces of deleted centes will be removed. If you remove based
...
Lets say you would only like to look at the recruitment plot for Bern.
...
ctu05_data_bern_berlin <- subset_secuTrial(ctu05_data, centre = c("Inselspital Bern (RPACK)",
                                                                  "Charité Berlin (RPACK)"))
# plot
...
There is a plethora of variables in the tables of secuTrial exports that start with `mnp`. These are meta variables
