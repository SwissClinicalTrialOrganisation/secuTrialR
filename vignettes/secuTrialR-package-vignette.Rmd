...
related to loading data exports from secuTrial into R. For this vignette and also to test your installation of the secuTrialR package
...
These values are missing on purpose, since missing values are needed to demonstrate some donwstream 
...
The code below shows how to load a secuTrial export. The output of load_secuTrial_export is a list. 

```{r}

sT_export_shortnames <- load_secuTrial_export(data_dir = export_location_shortnames)
typeof(sT_export_shortnames)
sT_export_longtnames <- load_secuTrial_export(data_dir = export_location_longnames)
...
The secuTrial export has aleady been loaded above. The code below shows how to load the validation overview.
...
that "age" is missing once while "grouping" and "bmd" are bith missing twice
...
correctly, knowledge of the study design is necessary. As expecteted the completeness is 
