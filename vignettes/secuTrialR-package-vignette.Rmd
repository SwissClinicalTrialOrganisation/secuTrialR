...
planning to add `secuTrialR` to CRAN but we are not there yet.
\vspace{5pt}
...
If you work on your own datasets you can specify a path as a regular character without
using `system.file()`.
...
and "sae") are present, but also that there are many tables which you do not recognize.

The majority of the unrecognizable tables are tagged as `TRUE` in the `meta` column.
This means that they are structural data tables. Their names and data structures
...
Furthermore, there is a set of tables whose names start with "at". These are audit trail
tables and only relevant if you need to investigate changes in the data.
...
the reading process and accessible via `gender.factor`. Thus, data is not overwritten
...
The device ID is returned to the console, but can be ignored. Forms are plotted in
...
fashion. The below function allows this for a loaded secuTrial data export.
\vspace{5pt}
...
Generally, it is advisable to recalculate score variables before data analysis.
...
During ongoing studies it is possible that changes to the secuTrial data entry
...
since they may not cover all alterations. Information is returned on new forms and
...
you might prefer doing this manually with the `haven` package.
...
with `mnp`. These are meta variables which are e.g. important to logically link the
different tables. Explaining them all is beyond the scope of this vignette.
For detailed explanations please refer to the secuTrial user manual.

# Disclaimer

The descriptions of the export data and the logic within are our understanding of 
them and come with no warranty. For in depth details please refer to the original
secuTrial manuals.

# The `sessionInfo()` of the vignette
