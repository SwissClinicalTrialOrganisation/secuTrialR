...
in your `secuTrialdata` object. To achieve this you can use the `merge()` function.
A common task may be to merge repetition form data to its parent form.
In our case `esurgeries` can be naively merged with `baseline` via the `mnpdocid`:
...
Please note, that the merging can cause duplication of data if the ids that the merge is directed by are not unique.
...
`mnpdocid` 234. Lets briefly illustrate this by looking at a `table()` of the `height` variable from the `baseline`
...
A better approach is to change the structure of your repetition form before merging. For this you need to
investigate which data you would like to merge. In our example case we are interested in the `surgery_organ`.
Of course we also need to drag the `mnpdocid` along to perform the actual merge.

\vspace{5pt}
```{r}
# write a temporary variable
...
During the merge we also set `all.x = TRUE` to prevent dropping rows from `baseline`
that have no match in `surg_wide`. Checking the dimensions reveals that the structure,
especially the line count, remains the same except for the data added from the `esurgeries`
repetition form.
...
The above description only provides a very brief and simplified example snapshot. In general, merging
