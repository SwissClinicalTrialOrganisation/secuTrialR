...
and "sae") are present, but also that there are many tables do not correspond to the
...
This `list` contains only empty entries. Thus, you can conclude that there are no new forms or
variables and no forms or variables have been removed.
...
Certain `warning` messages can occur quite frequently. Some of them may call for deliberate
action and thus it is important to understand them. We briefly mentioned some of them earlier in
this document but will now more closely explain how they can be interpreted.
...
The below warning may be triggered after running `read_secuTrial()`. It tells you that some
data in a date variable could not be converted during the process of date conversion (i.e. `dates_secuTrial()`).
This ususally occurs if incomplete date entries are present. Since the raw data is not overwritten but rather a
...
in the database. Because this interferes with the factorization we add the code
to the duplicate decoded value and return the below message to make you aware.
...
Duplicate values found during factorization of sys_therapy_detail")
```
